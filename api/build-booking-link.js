...
// Always returns: { success: true, url: "<booking URL>" } OR { success: true, url: "ERROR: <reason>" }

export default async function handler(req, res) {
  try {
    // Prevent caching by edge/CDN
    res.setHeader('Cache-Control', 'no-store');

    const API_KEY = process.env.CLOUDBEDS_API_KEY;

    // Allow override via request; else env
    const src     = req.method === 'POST' ? (req.body || {}) : (req.query || {});
    const BOOK_ID = (src.bookingId && String(src.bookingId)) || process.env.CLOUDBEDS_BOOKING_ID; // e.g., "svLoIs"
    ...
    // Local sanity checks (return ERROR in url)
    const nights = diffDays(checkin, checkout);
    if (!checkin || !checkout) return ok(res, `ERROR: Fechas inválidas: faltan check-in o check-out (YYYY-MM-DD).`);
    if (nights <= 0)          return ok(res, `ERROR: Fechas inválidas: el check-out debe ser posterior al check-in.`);
    if (!API_KEY)             return ok(res, `ERROR: Configuración del servidor faltante (API key).`);
    if (!BOOK_ID)             return ok(res, `ERROR: Falta configurar el ID del motor de reservas (bookingId).`);
    ...
    if (!resp.ok || json?.success === false || plans.length === 0) {
      return ok(res, `ERROR: No hay planes/tarifas disponibles para ese rango.`);
    }

    const details = Array.isArray(plans[0]?.roomRateDetailed) ? plans[0].roomRateDetailed : [];
    if (!details.length) return ok(res, `ERROR: No hay datos de tarifa para el rango solicitado.`);

    // Compute min stay (minLos)
    let minLos = 1;
    ...
    // Validation rules → return ERROR in url on failure
    if (windowDays.length < nights) return ok(res, `ERROR: No hay datos de tarifa para todas las noches solicitadas.`);
    if (nights < minLos)           return ok(res, `ERROR: La estadía mínima es de ${minLos} noches.`);
    if (arrival?.closedToArrival)  return ok(res, `ERROR: No se permite llegada en la fecha seleccionada.`);
    const depClosed =
      details.find(d => d?.date === checkout)?.closedToDeparture ||
      details.find(d => d?.date === lastNight)?.closedToDeparture || false;
    if (depClosed)                  return ok(res, `ERROR: No se permite salida en la fecha seleccionada.`);
    if (windowDays.some(d => toInt(d?.roomsAvailable, 0) === 0))
      return ok(res, `ERROR: No hay disponibilidad en una o más noches.`);
    if (windowDays.some(d => toNum(d?.rate, 0) <= 0))
      return ok(res, `ERROR: No hay tarifa publicada para una o más noches.`);

    // Build booking URL (valid)
    ...
    const bookingUrl = `https://hotels.cloudbeds.com/${LOCALE}/reservation/${BOOK_ID}/?${qs.toString()}`;

    return ok(res, bookingUrl);

  } catch {
    return ok(res, `ERROR: Error al procesar la solicitud.`);
  }
}

// — helper: respond with only { success, url } —
// If invalid, url is "ERROR: <reason>"; if valid, it's the booking link.
function ok(res, urlStr) {
  const payload = { success: true, url: String(urlStr || '') };
  return res
    .status(200)
