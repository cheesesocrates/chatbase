...
// Validates the stay, and only returns a link if valid.
// Always HTTP 200 with { status: "success", data: { ... } }

export default async function handler(req, res) {
  try {
    const API_KEY = process.env.CLOUDBEDS_API_KEY;
    // allow override via query/body; else use env
    ...
    // Query Cloudbeds getRatePlans (detailed)
    ...
    // Rules: min stay → closed arrival/departure → availability → nightly rates
    if (nights < minLos) {
...
function ok(res, body) {
  // Wrap so Chatbase shows values under `data` without mappings
