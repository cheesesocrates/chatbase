...
function findTypesArray(p){ if (Array.isArray(p?.data)) return p.data; if (Array.isArray(p?.roomTypes)) return p.roomTypes; if (Array.isArray(p?.availableRoomTypes)) return p.availableRoomTypes; if (Array.isArray(p?.rooms)) return p.rooms; return null; }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (new Date(endDate) <= new Date(startDate)) return res.status(400).json({ success:false, message:'endDate must be after startDate.' });
    ...
    if (!usedUrl) {
      return res.status(502).json({
        success:false,
        message:'Availability endpoint not found (404 on all variants).',
        tried
      });
    }
    ...
    if (!Array.isArray(list)) {
      return res.status(200).json({ success:true, canBook:false, reason:'Could not read availability list from Cloudbeds.', _endpoint: usedUrl, raw: payload });
    }

    const normalized = list.map(row => {
      const id = String(row.roomTypeID ?? row.room_type_id ?? row.id ?? '');
      ...
      return { roomTypeID:id, available, maxGuests };
    });

    const party = adults + children;
    const match = roomTypeID
      ? normalized.find(x => x.roomTypeID === roomTypeID && x.available >= quantity && (x.maxGuests == null || party <= x.maxGuests * quantity))
      : normalized.find(x => x.available >= quantity && (x.maxGuests == null || party <= x.maxGuests * quantity));

    return res.status(200).json({ success:true, canBook:Boolean(!!match), reason: match ? 'Available.' : 'Not available.', details:{ adults, children, quantity, match }, _endpoint: usedUrl });
  } catch (err) {
