...
function findTypesArray(p){
  if (Array.isArray(p?.data)) return p.data;
  if (Array.isArray(p?.roomTypes)) return p.roomTypes;
  if (Array.isArray(p?.availableRoomTypes)) return p.availableRoomTypes;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Try a few Cloudbeds paths (host/casing/version differ across accounts)
    const bases = [
    ...
    if (!usedUrl) return res.status(502).json({ success:false, message:'Availability endpoint not found (404 on all variants).' });
    ...
    // Ensure we never crash without a message
